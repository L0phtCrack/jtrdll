...
void device_format_reset()
{
	...
	unsigned int keys_per_crypt = jtr_bitstream->candidates_per_crypt / mask_num_cand();
	if (!keys_per_crypt)
		keys_per_crypt = 1;
	...
	MEM_FREE(keys_buffer);
	keys_buffer = mem_alloc(plaintext_len * jtr_fmt_params->max_keys_per_crypt);
	...
	MEM_FREE(range_info_buffer);
	if (!mask_is_inactive())
		range_info_buffer = mem_alloc(MASK_FMT_INT_PLHDR * jtr_fmt_params->max_keys_per_crypt);
...
int device_format_crypt_all(int *pcount, struct db_salt *salt)
{
	...
	for (;;)
	{
		...
		for (;;)
		{
			struct jtr_device *dev = jtr_device_list_process_inpkt(task_list);
			if (!dev)
				break;
