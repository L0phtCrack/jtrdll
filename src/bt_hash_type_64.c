...
BT_INLINE unsigned int modulo64_31b(uint64_t a, unsigned int N)
{
	return (unsigned int)(a % N);
}

BT_INLINE uint64_t add64(uint64_t a, unsigned int b)
{
	return (a + b);
}
...
BT_INLINE unsigned int calc_ht_idx_64(unsigned int hash_location, unsigned int offset)
{
	return  modulo64_31b(add64(loaded_hashes_64[hash_location], offset), hash_table_size);
}

BT_INLINE unsigned int zero_check_ht_64(unsigned int hash_table_idx)
{
	return (hash_table_64[hash_table_idx] || hash_table_64[hash_table_idx + hash_table_size]);
}

BT_INLINE void assign_ht_64(unsigned int hash_table_idx, unsigned int hash_location)
{
...
BT_INLINE void assign0_ht_64(unsigned int hash_table_idx)
{
