...
unsigned int modulo128_31b(uint128_t a, unsigned int N, uint64_t shift64)
{
	uint64_t p;
	p = (a.HI64 % N) * shift64;
	p += (a.LO64 % N);
...
uint128_t add128(uint128_t a, unsigned int b)
{
	uint128_t result;
...
unsigned int calc_ht_idx_128(unsigned int hash_location, unsigned int offset)
{
	return  modulo128_31b(add128(loaded_hashes_128[hash_location], offset), hash_table_size, shift64_ht_sz);
}

unsigned int zero_check_ht_128(unsigned int hash_table_idx)
{
...
void assign_ht_128(unsigned int hash_table_idx, unsigned int hash_location)
{
...
void assign0_ht_128(unsigned int hash_table_idx)
{
